...
def main():
    ...
    # dataloaders
    dataset = create_dataset(dataset_name=args.dataset, setting="Pretrain", root=args.data_dir, img_size=args.input_size[-1])
    trainset, _, testset = dataset.get_datasets()
    ...
    save_path = os.path.join("./save", args.model, args.dataset)
    print(f"Save result at {save_path}")
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    weights_path = os.path.join(save_path, f"{args.model}-{args.dataset}.pth.tar")
